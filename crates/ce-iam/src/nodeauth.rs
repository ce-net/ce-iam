...
/// A browser's vouch request (`{ peerId, name, nonce }`, camelCase to match `account.js`).
#[derive(Deserialize)]
...
impl NodeAuthResponder {
    ...
    /// Mint a node-signed vouch capability binding `peer_id` + `name`. Returns its hex token.
    pub fn vouch_token(&self, peer_id: &str, name: &str) -> Result<String> {
        let actions = vec![
    ...
    /// Handle one decoded vouch request: mint the cap and publish the response.
    async fn handle_req(&self, payload: &[u8]) -> Result<()> {
        ...
        let token = self.vouch_token(&req.peer_id, &name)?;
        let resp = json!({
            "nonce": req.nonce,
            "cap": token,
            "nodeId": self.node_id_hex,
        ...
        info!(peer = %req.peer_id, %name, "vouched (node-signed capability issued)");
        Ok(())
    }
