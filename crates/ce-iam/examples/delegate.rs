...
fn main() -> anyhow::Result<()> {
    let iam =
        Iam::new().with_action_universe(["storage:read".to_string(), "storage:write".to_string()]);
    ...
    let g1 = iam
        .mint(&org, Principal(alice.node_id()), &root_policy, 1)
        .map_err(anyhow_err)?;
    println!(
        "minted root grant for alice ({} bytes token)",
        g1.token.len()
    );
    ...
    let g2 = iam
        ...
        .map_err(anyhow_err)?;
    println!(
        "alice delegated a narrower grant to bob (depth {})",
        g2.chain.len()
    );
    ...
    let after_revoke = iam
        ...
        .is_ok();
    println!(
        "bob storage:read after root revoked -> {}",
        if after_revoke {
            "ALLOW (BUG)"
        } else {
            "DENY (correct)"
