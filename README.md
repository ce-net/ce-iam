...
## Two crates: a small core and the full IAM

This repo is a **workspace** with two member crates. Most apps do not need a policy engine or the
authority to mint capabilities — they need to verify a presented capability, hold/recover secrets,
and enroll their own devices. That is the **small** crate; the **big** crate adds the heavy IAM
machinery on top and re-exports the small one, so existing `ce_iam::*` paths keep working unchanged.

| crate | role | depends on |
|---|---|---|
| **`ce-iam-core`** (`crates/ce-iam-core`) | "basic auth", minimal deps, fast compile: identity + capability **VERIFY** (re-export `ce-cap`/`ce-identity`) + device enrollment (`device::DeviceStore`) + the **secrets vault** (`secrets::Vault`). | `ce-cap`, `ce-identity`, `ce-secrets-rs` (+ serde/anyhow) |
| **`ce-iam`** (`crates/ce-iam`) | full IAM: **mint/attenuate** (`grant`), policy/roles, catalog, wallet, roots, revocation, the device→cap **bridge** (which mints), and the unified CLI. Re-exports `ce-iam-core`. | `ce-iam-core`, `ce-cap`, `ce-identity`, `ce-rs`, clap, reqwest |

Rule: **device + secrets + cap-VERIFY live in `ce-iam-core`; minting/policy/roles live in `ce-iam`.**
An app that only verifies caps and holds secrets depends on the small crate and never compiles the IAM.

### `ce-iam-core::secrets` — the vault

`secrets::Vault<S: Store>` is the Rust port of `ce-secrets/src/vault.mjs`, built on the byte-exact JS
crypto in `ce-secrets-rs` and verified against golden vectors (`crates/ce-iam-core/tests/golden_secrets.rs`,
fixtures generated by `fixtures/gen_secrets_vectors.mjs` from the real `crypto.mjs`/`vault.mjs`).

- **Master**: derived deterministically from the owner's ECDH scalar (HKDF, salt `ce-vault:<ns>`,
  info `master-v1`) AND ECIES-wrapped per device — recoverable by the owner alone, readable by every
  enrolled device.
- **Storage is pluggable**: the vault is generic over an async `Store { get, put, del, list }`.
  `MemStore` backs tests; production points the same vault at a mesh KV (ce-coord / ce-kv).
- **Operations**: `init` / `recover`, `request_pairing` / `approve_pairing`, `list_devices` /
  `revoke_device`, `put_secret` / `get_secret` / `list_secrets` / `fingerprint` / `delete_secret`,
  `issue_grant` / `verify_grant` / `list_grants` / `revoke_grant`, `sign_challenge` / `verify_auth`.
- Records are ECDSA-signed by the writing device; the canonicalization reproduces the JS
  `signRecord` allowlist quirk byte-for-byte (a sixth interop trap, see `ce-secrets-rs::records`).
